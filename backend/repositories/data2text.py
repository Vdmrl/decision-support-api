...
class ProjectData:
                    ...
                    if user_projects:
                        for user_project in user_projects:
                            ...
                            if user:
                                ...
                                logger.info("No user in data_to_text")
                    else:
                        logger.warning("no user_project in data_to_text")
                else:
                    logger.warning("no project in data_to_text")
        except (SQLAlchemyError, Exception) as e:
            if isinstance(e, SQLAlchemyError):
                msg = "Database exception"
            else:
                msg = "Unknown exception"
            msg += ": can't denormalize account data into text"
            extra = {
            ...
            async with async_session_factory() as session:
                ...
                if project:
                    ...
                    if datas:
                        ...
                        logger.warning("no project_data in data_to_text")
                else:
                    logger.warning("no project in data_to_text")
        except (SQLAlchemyError, Exception) as e:
            if isinstance(e, SQLAlchemyError):
                ...
                msg = "Unknown exception"
            msg += ": can't denormalize project data into text"
            extra = {
                "id_project": id_project
            }
            logger.error(msg, extra=extra, exc_info=True)
            ...
            async with async_session_factory() as session:
                ...
                if project:
                    ...
                    if passport_datas:
                        ...
                        logger.warning("no passport_data in data_to_text")
                else:
                    logger.warning("no project in data_to_text")
        except (SQLAlchemyError, Exception) as e:
            if isinstance(e, SQLAlchemyError):
                ...
                msg = "Unknown exception"
            msg += ": can't denormalize project passport into text"
            extra = {
                "id_project": id_project
            }
            logger.error(msg, extra=extra, exc_info=True)
            ...
            async with async_session_factory() as session:
                ...
                if project:
                    ...
                    if event:
                        ...
                        logger.warning("no event in data_to_text")
                else:
                    logger.warning("no project in data_to_text")
        except (SQLAlchemyError, Exception) as e:
            if isinstance(e, SQLAlchemyError):
                ...
                msg = "Unknown exception"
            msg += ": can't denormalize event into text"
            extra = {
                "id_project": id_project
            }
            logger.error(msg, extra=extra, exc_info=True)
    ...
    @staticmethod
    async def is_in_db(id_project: int) -> bool:
        """
        ...
        try:
            ...
            async with async_session_factory() as session:
                query = select(Projects).filter(Projects.id_projects == id_project)
                result = await session.execute(query)
            ...
            if isinstance(e, SQLAlchemyError):
                ...
                msg = "Unknown exception"
            msg += ": can't check if project id in db projects"
            extra = {
                "id_project": id_project
            }
            logger.error(msg, extra=extra, exc_info=True)
        ...
        try:
            async with async_session_factory() as session:
                ...
                if support:
                    ...
                    logger.warning("no support in data_to_text")
        except (SQLAlchemyError, Exception) as e:
            if isinstance(e, SQLAlchemyError):
                msg = "Database exception"
            else:
                msg = "Unknown exception"
            msg += ": can't denormalize support measures data into text"
            extra = {
                "id_project": id_support
            }
            logger.error(msg, extra=extra, exc_info=True)
            ...
            async with async_session_factory() as session:
                ...
                if support:
                    ...
                    if institute:
                        ...
                        logger.warning("no institute in data_to_text")
                else:
                    logger.warning("no support in data_to_text")
        except (SQLAlchemyError, Exception) as e:
            if isinstance(e, SQLAlchemyError):
                ...
                msg = "Unknown exception"
            msg += ": can't denormalize institutes data into text"
            extra = {
                "id_project": id_support
            }
            logger.error(msg, extra=extra, exc_info=True)
        ...
        try:
            async with async_session_factory() as session:
                ...
                if supports:
                    ...
                    logger.warning("no supports in data_to_text")
        except (SQLAlchemyError, Exception) as e:
            if isinstance(e, SQLAlchemyError):
                msg = "Database exception"
            else:
